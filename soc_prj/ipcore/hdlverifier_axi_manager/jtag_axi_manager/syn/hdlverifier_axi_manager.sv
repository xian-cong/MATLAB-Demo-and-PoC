...
`timescale 1 ns / 1 ns
module hdlverifier_axi_manager #
(
...
logic [AXI_ADDR_WIDTH-1:0] address_async_stage1;
logic [AXI_ADDR_WIDTH-1:0] address_async_stage2;
reg  start_read;
reg  start_write;
reg  [7:0] burst_length;
reg  [7:0] burst_length_async_stage1;
reg  [7:0] burst_length_async_stage2;
reg  [1:0] burst_type;
reg  [1:0] burst_type_async_stage1;
reg  [1:0] burst_type_async_stage2;

logic [3:0] aclk_cnt;


// Double flip-flop for synthronization
logic start_read_async_stage1;
logic start_read_async_stage2;
logic start_write_async_stage1;
logic start_write_async_stage2;

wire  start_read_aclk;
logic start_read_aclk_d;
assign start_read_aclk = start_read_async_stage2;

wire  start_write_aclk;
logic start_write_aclk_d;
assign start_write_aclk = start_write_async_stage2;

// Assign write address channel constant values
assign axi4m_awid       = 0;
assign axi4m_arid       = 0;
assign axi4m_awlen      = burst_length_async_stage2; 
...
assign axi4m_arlen      = burst_length_async_stage2;
generate
...
assign axi4m_awaddr = address_async_stage2;

assign axi4m_bready = 1'b1;


assign axi4m_araddr = address_async_stage2;
assign axi4m_arprot = 3'b000;
assign axi4m_awprot = 3'b000;
assign axi4m_awburst = burst_type_async_stage2;
assign axi4m_arburst = burst_type_async_stage2;
    
localparam [7:0] MAJOR_VER = 8'b00000010;
...
logic tck_reset_async_stage1,tck_reset_async_stage2;
logic aresetn_d1,aresetn_d2;
logic aclk_reset;
always_ff@(posedge aclk) begin
    tck_reset_async_stage1 <= tck_reset;
    tck_reset_async_stage2 <= tck_reset_async_stage1;
    aresetn_d1 <= aresetn;
    aresetn_d2 <= aresetn_d1;
    aclk_reset <= tck_reset_async_stage2 || !aresetn_d2;
end
...
else if ( AXI_ADDR_WIDTH == 40 ) begin
   always@(posedge tck) begin
         if(tck_reset) 
              address <= 40'b0;
         else if(reg_write) begin
                if(reg_addr==5'd1)   // Lower 32 bits are driven here 
                    ...
                    address[39:32] <= reg_wdata;  // Upper 32 bits are driven here
                else if(reg_addr==5'd4) begin
                    burst_length <= reg_wdata[7:0];   
                    burst_type   <= reg_wdata[9:8];
                end
...
always_ff@(posedge aclk) begin      
    start_read_async_stage1 <= start_read;
    start_read_async_stage2 <= start_read_async_stage1;
    start_read_aclk_d <= start_read_aclk;
    burst_length_async_stage1 <= burst_length;
    burst_length_async_stage2 <= burst_length_async_stage1;
    address_async_stage1    <= address;
    address_async_stage2    <= address_async_stage1;
    burst_type_async_stage1 <= burst_type;
    burst_type_async_stage2 <= burst_type_async_stage1;
end

assign status[2] = !aresetn;
assign status[3] = !aclk_cnt[3];

always_ff@(posedge aclk or posedge aresetn_d1) begin
	if(aresetn_d1) // tck_reset
		aclk_cnt <= 4'b0;
...
always@(posedge aclk) begin
    ...
    else if(start_read_aclk && !start_read_aclk_d) begin
        axi4m_arvalid <= 1'b1;
        axi4m_rready <= 1'b0;
        status[0] <= 1'b0;
        read_count <= 'b0;
...
always@(posedge aclk) begin
    start_write_async_stage1 <= start_write;
    start_write_async_stage2 <= start_write_async_stage1;
    start_write_aclk_d <= start_write_aclk;
end
...
always@(posedge aclk) begin
    ...
    else if(start_write_aclk && !start_write_aclk_d && state_write==WSTATE_IDLE) begin
...
assign axi4m_wlast = (write_word_count == burst_length_async_stage2) && axi4m_wvalid;
