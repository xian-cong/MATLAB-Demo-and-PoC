...
`timescale 1 ns / 1 ns
module hdlverifier_axi_manager_versal #
(
...
// Versal Bscan signals
    input  wire        versal_capture,
    input  wire        versal_jtag_reset,
    input  wire        versal_sel,
    input  wire        versal_shift,
    input  wire        versal_tck,
    input  wire        versal_tdi,
    input  wire        versal_update,
    output reg         versal_tdo	
);
...
// "MAJOR_VER" , "MINOR_VER" are useful to identify the Sowftware is in sync with HDL IP.
// "APP_TYPE" is depends on application for AXIM it is '0', software(MAC) read this value while establishing channel.
// "SIGNATURE", this parameter also to athenticate before establishing channel.
...
always@(posedge aclk) begin
    ...
    // waiting for start of write trasaction
    else if(start_write_d1 && !start_write_d2 && state_write==WSTATE_IDLE) begin
        state_write <= WSTATE_RFIRST;
    end
    // make awvalid and wvalid to "high"
        ...
        // slave is ready to accept the data in specified location.
        if(axi4m_awready && axi4m_wready && axi4m_wlast) begin
            state_write <= WSTATE_WAIT_BVALID;
            axi4m_awvalid <= 1'b0;
            axi4m_wvalid <= 1'b0;
        end
        // slave confirmed the address but not ready to receive data.
        else if(axi4m_awready) begin
            state_write <= WSTATE_WAIT_WREADY;
            axi4m_awvalid <= 1'b0;
        end
        // slave is ready to accept the data but address is not confirmed. 
        else if(axi4m_wready && axi4m_wlast) begin
            state_write <= WSTATE_WAIT_AWREADY;
            axi4m_wvalid <= 1'b0;
        end
    end
    // waiting for slave wready to trasfer the data.
    ...
    // watting for address to confirm
    ...
    // waiting for bvalid signal from slave.
